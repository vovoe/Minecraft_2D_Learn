#include <SFML/Graphics.hpp>
#include "Title.h"
#include "Sound.h"
#pragma once

class Player {
public:
	float dx, dy;
	FloatRect rect;
	bool onGround, set;
	Sprite sprite;
	float curFrame, anim;
	int soud;

	Player(Texture& image) {
		sprite.setTexture(image);
		rect = FloatRect(40 * ts, 8 * ts, 80, 160);

		dx = dy = 0;
		curFrame = anim = soud = 0;
		set = false;
	}

	void update(float time, int pos) {
		rect.left += dx * time;
		Collision(0);

		if (!onGround)
			dy = dy + 0.0005 * time;

		rect.top += dy * time;

		onGround = false;
		Collision(1);

		if (!set) {
			curFrame += 0.01 * time;

			if (curFrame < 6)
				anim = curFrame;
			else {
				anim = 5 - (curFrame - 5);
			}

			if (curFrame > 8)
				curFrame = 1;

			if (dx > 0)
				sprite.setTextureRect(IntRect(80 * int(anim), 0, 80, 160));
			if (dx < 0)
				sprite.setTextureRect(IntRect(80 * int(anim) + 80, 0, -80, 160));
		}
		else {
			curFrame += 0.02 * time;

			if (curFrame < 5)
				anim = curFrame;
			else {
				anim = 5 - (curFrame - 5);
			}

			if (curFrame > 9)
				set = false;

			if (rect.left - offsetX < pos)
				sprite.setTextureRect(IntRect(80 * int(anim), 160, 80, 160));
			else
				sprite.setTextureRect(IntRect(80 * int(anim) + 80, 160, -80, 160));
		}

		sprite.setPosition(rect.left - offsetX, rect.top - offsetY);

		dx = 0;
	}

	void Collision(float dir) {
		for (int i = rect.top / ts; i < (rect.top + rect.height) / ts; i++)
			for (int j = rect.left / ts; j < (rect.left + rect.width) / ts; j++) {
				if (TileMap[i][j] == 'A' || TileMap[i][j] == 'B' || TileMap[i][j] == 'C' || TileMap[i][j] == 'D' ||
					TileMap[i][j] == 'F' || TileMap[i][j] == 'G' || TileMap[i][j] == 'H' || TileMap[i][j] == 'I' ||
					TileMap[i][j] == 'J' || TileMap[i][j] == 'K' || TileMap[i][j] == 'L' || TileMap[i][j] == 'M' ||
					TileMap[i][j] == 'X') {
					if (dx > 0 && dir == 0)
						rect.left = j * ts - rect.width;
					if (dx < 0 && dir == 0)
						rect.left = j * ts + ts;
					if (dy > 0 && dir == 1) {
						rect.top = i * ts - rect.height;
						dy = 0;
						onGround = true;

						if (dx != 0) {
							if (soud == 0)
								sound.update(TileMap[i][j]);

							soud++;

							if (soud == 25)
								soud = 0;
						}
					}
					if (dy < 0 && dir == 1) {
						rect.top = i * ts + ts;
						dy = 0;
					}
				}
			}
	}
};